...
const DialogDeleteLevel = () => {
  const { dialogDeleteLevel, handleDeleteLevel, deleteLevel, level } =
    useProfile();
  return (
    <Transition.Root show={dialogDeleteLevel} as={Fragment}>
      <Dialog
        as="div"
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  <div className="mt-2">
                    <p className="text-sm">
                      {level?.title && (
                        <span>
                          {`Are your sure you want to delete the '${level.title}' experience ?.`}
                        </span>
                      )}
                    </p>
                  </div>
...
export default DialogDeleteLevel;
