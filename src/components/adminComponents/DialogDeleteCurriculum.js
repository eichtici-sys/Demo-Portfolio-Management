...
const DialogDeleteCurriculum = () => {
  const { dialogDeleteCurriculum, handleDeleteCV, deleteCV, curriculum } =
    useProfile();
  return (
    <Transition.Root show={dialogDeleteCurriculum} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-40 inset-0 overflow-y-auto"
        onClose={handleDeleteCV}
      >
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6 w-full">
              ...
              <div className="sm:flex flex-col justify-center">
                <div className="flex gap-4 sm:gap-2">
                  <div className="xm:mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  <div className="mt-2">
                    <p className="text-sm">
                      {curriculum?.name && (
                        <span>
                          {`Are your sure you want to delete the curriculum: '${curriculum.name}' ?.`}
                        </span>
                      )}
                    </p>
                  </div>
...
export default DialogDeleteCurriculum;
