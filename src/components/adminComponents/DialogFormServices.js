...
const DialogFormServices = () => {
  ...
  const { dialogService, handleDialogService, submitService, service } =
    useService();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if ([name, description, icon].includes("")) {
      toast.error("All fields are required");
      return;
    }
    await submitService({ id, name, description, icon });
    setId("");
    setName("");
    setDescription("");
    setIcon("");
  ...
  return (
    <Transition.Root show={dialogService} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-40 inset-0 overflow-y-auto"
        onClose={handleDialogService}
      >
              ...
              <div className="sm:flex sm:items-start">
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  ...
                  <form className="mt-6 mb-4" onSubmit={handleSubmit}>
                    ...
                    <input
                      type="submit"
                      value={id ? "Save Changes" : "Create Service"}
