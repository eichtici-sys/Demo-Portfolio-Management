...
const DialogFormSkills = () => {
  const { handleDialogSkills, dialogFormSkill, submitSkill, skill } =
    useProfile();
  ...
  const handlesubmit = async (e) => {
    e.preventDefault();
    if ([name, scale].includes("")) {
      toast.error("All fields are required");
      return;
    }
    await submitSkill({ id, name, scale, level: router.query.id });
    setId("");
    setName("");
    setScale("");
  };
  ...
  return (
    <Transition.Root show={dialogFormSkill} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-40 inset-0 overflow-y-auto"
        onClose={handleDialogSkills}
      >
              ...
              <div className="sm:flex sm:items-start">
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  ...
                  <form className="mt-6 mb-4" onSubmit={handlesubmit}>
                    ...
                    <input
                      type="submit"
                      value={id ? "Save Changes" : "Add Skill"}
...
export default DialogFormSkills;
