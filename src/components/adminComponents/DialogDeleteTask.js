...
const DialogDeleteTask = () => {
  const { dialogDeleteTask, handleDeleteTask, deleteTask, task } = useProject();
  return (
    <Transition.Root show={dialogDeleteTask} as={Fragment}>
      <Dialog
        as="div"
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  <div className="mt-2">
                    <p className="text-sm">
                      {task?.name && (
                        <span>
                          {`Are your sure you want to delete the '${task.name}' ?.`}
                        </span>
                      )}
                    </p>
                  </div>
