...
const DialogFormLevel = () => {
  const [id, setId] = useState("");
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [icon, setIcon] = useState("");

  const { dialogFormLevel, handleModalLevel, submitLevel, level } =
    useProfile();
  ...
  const handlesubmit = async (e) => {
    e.preventDefault();
    if ([title, description, icon].includes("")) {
      toast.error("All fields are required");
      return;
    }
    await submitLevel({ id, title, description, icon });
    setId("");
    setTitle("");
    setDescription("");
    setIcon("");
  ...
  return (
    <Transition.Root show={dialogFormLevel} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-40 inset-0 overflow-y-auto"
        onClose={handleModalLevel}
      >
              ...
              <div className="sm:flex sm:items-start">
                <div className="mt-3 sm:mt-0 w-full">
                  ...
                  <form className="mt-6 mb-4" onSubmit={handlesubmit}>
                    ...
                    <input
                      type="submit"
                      value={id ? "Save Changes" : "Create Level"}
...
export default DialogFormLevel;
