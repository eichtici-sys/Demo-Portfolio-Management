...
const DialogChangeImgPresentation = () => {
  const {
    about,
  ...
  const [infoImagePresentation, setInfoImagePresentation] = useState("");
  ...
  useEffect(() => {
    ...
    if (about._id) {
      setId(about._id);
      setImageURL(about.imagePresentationURL);
    }
    setInfoImagePresentation("");
  ...
  const onSelectFile = (e) => {
    ...
    if (selectFile) {
      setInfoImagePresentation(selectFile);
      setNameFile(selectFile.name);
    }
  };

  useEffect(() => {
    if (!infoImagePresentation) {
      setPreview(undefined);
      return;
    }
    const objectUrl = URL.createObjectURL(infoImagePresentation);
    setPreview(objectUrl);
    return () => URL.revokeObjectURL(objectUrl);
  }, [infoImagePresentation]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (infoImagePresentation.type === "image/gif") {
      toast.error("Image Not supported");

      return;
    }
    await changeImagePresentation({ id, infoImagePresentation });
    setInfoImagePresentation("");
    setPreview(undefined);
    setNameFile("");
    setImageURL("");
  ...
  return (
    <Transition.Root show={dialogChangeImagePresentation} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-40 inset-0 overflow-y-auto"
        onClose={handleChangeImagePresentation}
      >
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6 w-full">
              <div className="hidden sm:block absolute top-0 right-0 pt-4 pr-4">
                <button
                  ...
                  onClick={handleChangeImagePresentation}
                >
...
export default DialogChangeImgPresentation;
