...
const DialogChangeImgBackground = () => {
  const {
    about,
  ...
  const [infoImageBackground, setInfoImageBackground] = useState("");
  ...
  useEffect(() => {
    ...
    if (about._id) {
      setId(about._id);
      setImageURL(about.imageBG_URL);
    }
    setInfoImageBackground("");
  ...
  const onSelectFile = (e) => {
    ...
    if (selectFile) {
      setInfoImageBackground(selectFile);
      setNameFile(selectFile.name);
    }
  };

  useEffect(() => {
    if (!infoImageBackground) {
      setPreview(undefined);
      return;
    }
    const objectUrl = URL.createObjectURL(infoImageBackground);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (infoImageBackground.type === "image/gif") {
      toast.error("Image Not supported");
      return;
    }
    await changeImageBackground({ id, infoImageBackground });
    setInfoImageBackground("");
    setPreview(undefined);
    setNameFile("");
    setImageURL("");
  ...
  return (
    <Transition.Root show={dialogChangeImageBackground} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-40 inset-0 overflow-y-auto"
        onClose={handleChangeImageBackground}
      >
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6 w-full">
              <div className="hidden sm:block absolute top-0 right-0 pt-4 pr-4">
                <button
                  ...
                  onClick={handleChangeImageBackground}
                >
...
export default DialogChangeImgBackground;
