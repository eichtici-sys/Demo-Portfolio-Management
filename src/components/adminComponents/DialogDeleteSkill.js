...
const DialogDeleteSkill = () => {
  const { dialogDeleteSkill, handleDeleteSkill, deleteSkill, skill } =
    useProfile();
  return (
    <Transition.Root show={dialogDeleteSkill} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-40 inset-0 overflow-y-auto"
        onClose={handleDeleteSkill}
      >
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  <div className="mt-2">
                    <p className="text-sm">
                      {skill?.name && (
                        <span>
                          {`Are your sure you want to delete the '${skill.name}' ?.`}
                        </span>
                      )}
                    </p>
                  </div>
...
export default DialogDeleteSkill;
