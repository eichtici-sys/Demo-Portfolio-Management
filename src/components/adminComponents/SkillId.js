...
import DialogFormSkills from "./DialogFormSkills";
import DialogDeleteSkill from "./DialogDeleteSkill";
import Breadcrumb from "./Breadcrumb";
import Nodata from "./Nodata";
import useProfile from "@/hooks/useProfile";
import { formatDate } from "@/helpers/formatDate";
import { UilPlus } from "@iconscout/react-unicons";
import { skillColumns } from "@/data/dataColumns";

const SkillId = () => {
  const { level, getLevelById, handleDialogSkills, skills } = useProfile();
  ...
  }, [router.isReady, router.query, skills, getLevelById, id]);
        ...
        <div className="flex lg:flex-row flex-col gap-2 md:gap-4 px-0 md:px-4">
          <div className="w-full lg:w-1/3 px-3 md:px-7 py-3 border-4 border-primary1 rounded-xl">
            ...
            <div className="pt-5 pb-3 flex flex-col gap-3">
              ...
              <p>
                <span className="font-semibold block">Icon: </span>
                &ldquo;{level.icon}&rdquo;
              </p>
              <p>
                <span className="font-semibold block">Created By: </span>
                {level?.user?.userProfile?.name}
              </p>
              <p>
          ...
          <div className="flex-1 px-0 lg:px-7 py-3 bg-white rounded-md ">
            ...
            <div className="pt-4">
              {level.skills?.length ? (
                <TableGrid rows={level.skills} columns={skillColumns} />
              ) : (
                <Nodata
                  text={"There are no data. Add a skill to see it here"}
                />
              )}
            </div>
          </div>
