...
const DialogChangeImgAbout = () => {
  ...
  const [infoImageAbout, setInfoImageAbout] = useState("");
  ...
  useEffect(() => {
    ...
    if (about._id) {
      setId(about._id);
      setImageURL(about.imageAboutURL);
    }
    setInfoImageAbout("");
  ...
  useEffect(() => {
    ...
    const objectUrl = URL.createObjectURL(infoImageAbout);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (infoImageAbout.type === "image/gif") {
      toast.error("Image Not supported");
      return;
    }
    await changeImageAbout({ id, infoImageAbout });
    setInfoImageAbout("");
    setPreview(undefined);
    setNameFile("");
    setImageURL("");
  ...
  return (
    <Transition.Root show={dialogChangeImageAbout} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-40 inset-0 overflow-y-auto"
        onClose={handleChangeImageAbout}
      >
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6 w-full">
              <div className="hidden sm:block absolute top-0 right-0 pt-4 pr-4">
                <button
                  ...
                  onClick={handleChangeImageAbout}
                >
...
export default DialogChangeImgAbout;
