...
const DialogDeleteService = () => {
  const { dialogDeleteService, handleDeleteService, deleteService, service } =
    useService();
  return (
    <Transition.Root show={dialogDeleteService} as={Fragment}>
      <Dialog
        as="div"
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  <div className="mt-2">
                    <p className="text-sm">
                      {service?.name && (
                        <span>
                          {`Are your sure you want to delete the '${service.name}' ?.`}
                        </span>
                      )}
                    </p>
                  </div>
