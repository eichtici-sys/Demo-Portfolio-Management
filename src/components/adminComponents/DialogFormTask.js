...
const DialogFormTask = () => {
  const { handleDialogTask, dialogFormTask, submitTask, task } = useProject();
  ...
  const handlesubmit = async (e) => {
    ...
    await submitTask({ id, name, description, project: router.query.id });
    setId("");
    setName("");
    setDescription("");
  };
  ...
  return (
    <Transition.Root show={dialogFormTask} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-40 inset-0 overflow-y-auto"
        onClose={handleDialogTask}
      >
